package org.moss.lunar.image.palette;
...
public class PaletteProcess
{
	...
	public Float convertRgb(String[] rgbString) throws Exception
	{
		...
		} else
		{
			int[] rgb = new int[]
			{ red, green, blue };
			return searchRgb(rgb);
		}
	}

	public Float convertRgb(int[] rgb) throws InterpException
	{
		if (evaluationMap.containsKey(rgb[0], rgb[1], rgb[2]))
		{
			return (Float) evaluationMap.get(rgb[0], rgb[1], rgb[2]);
		} else
		{
			return searchRgb(rgb);
		}
									...
									boolean green, boolean blue)
	{
		...
		for (int i = 0; i < sectionRgb.size(); i++)
		{
			...
			RgbDto currentRgb = new RgbDto(rgb[0], rgb[1], rgb[2]);
